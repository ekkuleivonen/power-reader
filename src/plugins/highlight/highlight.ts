...
const toggleHighlights = (msg: { toggle: string; value: boolean }) => {
  if (
    (msg.toggle === "highlights" || msg.toggle === "highlights_all") &&
    msg.value === true
  )
    return highlightWords(paragraphs); //turn on
  if (
    (msg.toggle === "highlights" || msg.toggle === "highlights_all") &&
    msg.value === false
  )
    return deHighlightWords(); //turn off
};
//TODO: check local storage for all pages selection????

//talk with popup and bg scripts
chrome.runtime.onMessage.addListener(
  (msg: { toggle: string; value: boolean }, sender, response) => {
    if (msg.toggle === "highlights" || msg.toggle === "highlights_all")
      return toggleHighlights(msg);
    //if (msg.toggle === "reader_mode") return toggleReaderMode(msg);
  }
);

const plugin = "highlights";
